...
/**
 *    @file
 *      This file implements a parser for the Matter TLV (Tag-Length-Value) encoding format.
 *
 */
 ...
 * kProfileIdNotSpecified, the reader will return a #MATTER_ERROR_UNKNOWN_IMPLICIT_TLV_TAG error.
 */
...
/**
 * @typedef MATTER_ERROR (*TLVReader::GetNextBufferFunct)(TLVReader& reader, uintptr_t& bufHandle, const uint8_t *& bufStart, uint32_t& bufLen)
 *
 ...
 *                                  to 0.
 *
 * @retval #MATTER_NO_ERROR          If the function successfully produced more TLV data, or the end of
 *                                  the input data was reached (@p bufLen should be set to 0 in this case).
 * @retval other                    Other Matter or platform-specific error codes indicating that an error
 ...
 *
 * @retval #MATTER_NO_ERROR              If the method succeeded.
 * @retval #MATTER_ERROR_WRONG_TLV_TYPE  If the current element is not a TLV boolean type, or the
 *                                      reader is not positioned on an element.
 *
 */
MATTER_ERROR TLVReader::Get(bool& v)
{
    TLVElementType elemType = ElementType();
    if (elemType == kTLVElementType_BooleanFalse)
        ...
        return MATTER_ERROR_WRONG_TLV_TYPE;
 ...
 *
 * @retval #MATTER_NO_ERROR              If the method succeeded.
 * @retval #MATTER_ERROR_WRONG_TLV_TYPE  If the current element is not a TLV integer type (signed or
 *                                      unsigned), or the reader is not positioned on an element.
 *
 */
MATTER_ERROR TLVReader::Get(int8_t& v)
{
    uint64_t v64 = 0;
    MATTER_ERROR err = Get(v64);
    v = v64;
    return err;
}
 ...
 *
 */
MATTER_ERROR TLVReader::Get(int16_t& v)
{
    uint64_t v64 = 0;
    MATTER_ERROR err = Get(v64);
    v = v64;
 ...
 *
 */
MATTER_ERROR TLVReader::Get(int32_t& v)
{
    uint64_t v64 = 0;
    MATTER_ERROR err = Get(v64);
    v = v64;
 ...
 *
 */
MATTER_ERROR TLVReader::Get(int64_t& v)
{
    uint64_t v64 = 0;
    MATTER_ERROR err = Get(v64);
    v = v64;
 ...
 *
 */
MATTER_ERROR TLVReader::Get(uint8_t& v)
{
    uint64_t v64 = 0;
    MATTER_ERROR err = Get(v64);
    v = v64;
 ...
 *
 */
MATTER_ERROR TLVReader::Get(uint16_t& v)
{
    uint64_t v64 = 0;
    MATTER_ERROR err = Get(v64);
    v = v64;
 ...
 *
 */
MATTER_ERROR TLVReader::Get(uint32_t& v)
{
    uint64_t v64 = 0;
    MATTER_ERROR err = Get(v64);
    v = v64;
 ...
 *
 */
MATTER_ERROR TLVReader::Get(uint64_t& v)
{
    switch (ElementType())
    {
 ...
 *
 * @retval #MATTER_NO_ERROR              If the method succeeded.
 * @retval #MATTER_ERROR_WRONG_TLV_TYPE  If the current element is not a TLV floating point type, or
 *                                      the reader is not positioned on an element.
 *
 */
MATTER_ERROR TLVReader::Get(double& v)
{
    switch (ElementType())
    {
 ...
 *
 * @retval #MATTER_NO_ERROR              If the method succeeded.
 * @retval #MATTER_ERROR_WRONG_TLV_TYPE  If the current element is not a TLV byte or UTF8 string, or
 *                                      the reader is not positioned on an element.
 * @retval #MATTER_ERROR_BUFFER_TOO_SMALL
 *                                      If the supplied buffer is too small to hold the data associated
 *                                      with the current element.
 * @retval #MATTER_ERROR_TLV_UNDERRUN    If the underlying TLV encoding ended prematurely.
 * @retval other                        Other Matter or platform error codes returned by the configured
 *                                      GetNextBuffer() function. Only possible when GetNextBuffer is
 *                                      non-NULL.
 *
 */
MATTER_ERROR TLVReader::GetBytes(uint8_t *buf, uint32_t bufSize)
{
    if (!TLVTypeIsString(ElementType()))
        return MATTER_ERROR_WRONG_TLV_TYPE;

    if (mElemLenOrVal > bufSize)
        return MATTER_ERROR_BUFFER_TOO_SMALL;

    MATTER_ERROR err = ReadData(buf, (uint32_t) mElemLenOrVal);
 ...
 *
 */
MATTER_ERROR TLVReader::GetString(char *buf, uint32_t bufSize)
{
    if (!TLVTypeIsString(ElementType()))
        return MATTER_ERROR_WRONG_TLV_TYPE;
 ...
 * @retval #MATTER_ERROR_WRONG_TLV_TYPE  If the current element is not a TLV byte or UTF8 string, or
 *                                      the reader is not positioned on an element.
 * @retval #MATTER_ERROR_NO_MEMORY       If memory could not be allocated for the output buffer.
 * @retval #MATTER_ERROR_TLV_UNDERRUN    If the underlying TLV encoding ended prematurely.
 * @retval #MATTER_ERROR_UNSUPPORTED_MATTER_FEATURE
 ...
 *
 */
MATTER_ERROR TLVReader::DupBytes(uint8_t *& buf, uint32_t& dataLen)
{
    ...
    return MATTER_NO_ERROR;
#else
    return MATTER_ERROR_UNSUPPORTED_MATTER_FEATURE;
 ...
 *
 */
MATTER_ERROR TLVReader::DupString(char *& buf)
{
#if HAVE_MALLOC && HAVE_FREE
    if (!TLVTypeIsString(ElementType()))
        return MATTER_ERROR_WRONG_TLV_TYPE;
    ...
    MATTER_ERROR err = ReadData((uint8_t *) buf, (uint32_t) mElemLenOrVal);
    if (err != MATTER_NO_ERROR)
    {
        free(buf);
        return err;
 ...
 * Otherwise the method returns #MATTER_ERROR_TLV_UNDERRUN.  This makes the method of limited use when
 ...
 *
 * @retval #MATTER_NO_ERROR              If the method succeeded.
 * @retval #MATTER_ERROR_WRONG_TLV_TYPE  If the current element is not a TLV byte or UTF8 string, or the
 *                                      reader is not positioned on an element.
 * @retval #MATTER_ERROR_TLV_UNDERRUN    If the underlying TLV encoding ended prematurely or the value
 ...
 *
 */
MATTER_ERROR TLVReader::GetDataPtr(const uint8_t *& data)
{
    MATTER_ERROR err;
    ...
    err = EnsureData(MATTER_ERROR_TLV_UNDERRUN);
    if (err != MATTER_NO_ERROR)
        return err;
    ...
    if (remainingLen < (uint32_t)mElemLenOrVal)
        return MATTER_ERROR_TLV_UNDERRUN;
 ...
 * of the collection until the end is reached, at which point the reader will return MATTER_END_OF_TLV.
 *
 ...
 *
 * @retval #MATTER_NO_ERROR              If the method succeeded.
 * @retval #MATTER_ERROR_INCORRECT_STATE If the current element is not positioned on a container element.
 *
 */
MATTER_ERROR TLVReader::OpenContainer(TLVReader& containerReader)
{
    TLVElementType elemType = ElementType();
 ...
 *
 * @retval #MATTER_NO_ERROR              If the method succeeded.
 * @retval #MATTER_ERROR_INCORRECT_STATE If OpenContainer() has not been called on the reader, or if
 *                                      the container reader does not match the one passed to the
 *                                      OpenContainer() method.
 * @retval #MATTER_ERROR_TLV_UNDERRUN    If the underlying TLV encoding ended prematurely.
 * @retval #MATTER_ERROR_INVALID_TLV_ELEMENT
 *                                      If the reader encountered an invalid or unsupported TLV
 *                                      element type.
 * @retval #MATTER_ERROR_INVALID_TLV_TAG If the reader encountered a TLV tag in an invalid context.
 * @retval other                        Other Matter or platform error codes returned by the configured
 *                                      GetNextBuffer() function. Only possible when GetNextBuffer is
 *                                      non-NULL.
 *
 */
MATTER_ERROR TLVReader::CloseContainer(TLVReader& containerReader)
{
    MATTER_ERROR err;
 ...
 *
 */
MATTER_ERROR TLVReader::EnterContainer(TLVType& outerContainerType)
{
    TLVElementType elemType = ElementType();
    if (!TLVTypeIsContainer(elemType))
        return MATTER_ERROR_INCORRECT_STATE;
 ...
 *
 */
MATTER_ERROR TLVReader::ExitContainer(TLVType outerContainerType)
{
    MATTER_ERROR err;
 ...
 * Next() and checking for a return value of MATTER_END_OF_TLV.
 *
 * @note When there are more TLV elements in the collection, this method will change the position
 * of the reader.
 *
 * @retval #MATTER_NO_ERROR              If there are no further TLV elements to be read.
 * @retval #MATTER_ERROR_UNEXPECTED_TLV_ELEMENT
 ...
 *
 */
MATTER_ERROR TLVReader::VerifyEndOfContainer()
{
    MATTER_ERROR err = Next();
    if (err == MATTER_END_OF_TLV)
        return MATTER_NO_ERROR;
    if (err == MATTER_NO_ERROR)
        return MATTER_ERROR_UNEXPECTED_TLV_ELEMENT;
    return err;
}
 ...
 * return a #MATTER_END_OF_TLV error and the position of the reader will remain unchanged.
 *
 * @retval #MATTER_NO_ERROR              If the reader was successfully positioned on a new element.
 * @retval #MATTER_END_OF_TLV            If no further elements are available.
 * @retval #MATTER_ERROR_TLV_UNDERRUN    If the underlying TLV encoding ended prematurely.
 * @retval #MATTER_ERROR_INVALID_TLV_ELEMENT
 *                                      If the reader encountered an invalid or unsupported TLV element
 *                                      type.
 * @retval #MATTER_ERROR_INVALID_TLV_TAG If the reader encountered a TLV tag in an invalid context.
 * @retval #MATTER_ERROR_UNKNOWN_IMPLICIT_TLV_TAG
 ...
 * @retval #MATTER_NO_ERROR              If the reader was successfully positioned on a new element.
 * @retval #MATTER_END_OF_TLV            If no further elements are available.
 * @retval #MATTER_ERROR_WRONG_TLV_TYPE  If the type of the new element does not match the value
 ...
 *
 */
MATTER_ERROR TLVReader::Next(TLVType expectedType, uint64_t expectedTag)
{
    MATTER_ERROR err = Next();
    if (err != MATTER_NO_ERROR)
        return err;
 ...
 *
 * @retval #MATTER_NO_ERROR              If the reader was successfully positioned at the end of the
 *                                      data.
 * @retval other                        Other Matter or platform error codes returned by the configured
 *                                      GetNextBuffer() function. Only possible when GetNextBuffer is
 *                                      non-NULL.
 */
MATTER_ERROR TLVReader::SkipData(void)
{
...
MATTER_ERROR TLVReader::SkipToEndOfContainer()
{
    MATTER_ERROR err;
...
MATTER_ERROR TLVReader::ReadElement()
{
    MATTER_ERROR err;
    ...
    // Make sure we have input data. Return MATTER_END_OF_TLV if no more data is available.
    err = EnsureData(MATTER_END_OF_TLV);
    if (err != MATTER_NO_ERROR)
        return err;
    ...
    if (!IsValidTLVType(elemType))
        return MATTER_ERROR_INVALID_TLV_ELEMENT;
...
MATTER_ERROR TLVReader::VerifyElement()
{
    if (ElementType() == kTLVElementType_EndOfContainer)
    {
        ...
        if (mElemTag != AnonymousTag)
            return MATTER_ERROR_INVALID_TLV_TAG;
    }
    else
    {
        if (mElemTag == UnknownImplicitTag)
            return MATTER_ERROR_UNKNOWN_IMPLICIT_TLV_TAG;
...
MATTER_ERROR TLVReader::ReadData(uint8_t *buf, uint32_t len)
{
    MATTER_ERROR err;
...
MATTER_ERROR TLVReader::EnsureData(MATTER_ERROR noDataErr)
{
    MATTER_ERROR err;
 ...
 */
MATTER_ERROR TLVReader::GetElementHeadLength(uint8_t& elemHeadBytes) const
{
    MATTER_ERROR err = MATTER_NO_ERROR;
    uint8_t tagBytes;
    ...
    VerifyOrExit(IsValidTLVType(elemType), err = MATTER_ERROR_INVALID_TLV_ELEMENT);
